...
const loadFromServer = async () => {
    loading.value = true;
    const response = await fetch(route('companyAdmin.all-users', { ...serverOptions.value, search: search.value }));
...
const toggleUserBlockStatus = async (user) => {
    axios.put(route('companyAdmin.users.toggle-block-status', user.id)).then(response => {
...
const softDeleteUser = async (user) => {
    if (confirm("Are you sure you want to delete this user?")) {
        axios.delete(route('companyAdmin.users.softDelete', user.id)).then(response => {
            if (response.data.success) {
                toast.success(response.data.message);
                loadFromServer();
            }
